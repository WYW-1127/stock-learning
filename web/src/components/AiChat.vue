...
const messages = ref([]); // {role:'user'|'assistant', text, reply?}
...
async function send(preset) {
  ...
  try {
    ...
    if (r.ok) {
      messages.value.push({ role: 'assistant', text: r.reply.suggestion, reply: r.reply });
    } else {
      messages.value.push({ role: 'assistant', text: r.reason || 'AI 暂不可用', error: true, notConfigured: r.notConfigured });
    }
  } catch {
    messages.value.push({ role: 'assistant', text: '网络异常,请稍后再试。', error: true });
  } finally {
    sending.value = false;
    scrollBottom();
  }
      ...
      <div v-else ref="listEl" class="chat-list">
        ...
        <template v-for="(m, i) in messages" :key="i">
          ...
          <div v-else class="msg ai">
            ...
            <div v-else :class="{ 'ai-err': m.error }">{{ m.text }}</div>
            <div class="ai-disclaimer">AI 仅供学习参考,不构成投资建议</div>
          </div>
        </template>

        <div v-if="sending" class="msg ai thinking">
...
.thinking em { font-size: 12px; margin-left: 4px; }
