...
T2 交易引擎完成(38/38 单测全绿);**下一步:T3 本地存储(store.js)**。
...
1. **T3 本地存储(当前任务)**— server/src/store.js:account/positions/trades/watchlist/snapshots 读写;临时文件+rename 原子写;.bak 备份;启动损坏恢复;reset;store 单测
2. T4 行情服务:腾讯实时(GBK)+ 东财 K线/搜索 + 缓存重试 + mock 测试
3. T5 API 层组装:全部路由 + 下单串行队列
4. T6 前端五屏按设计稿实现(klinecharts 真实K线)
5. T7 联调收尾:启动.bat 打磨、手动清单回归

> 用户指示:T2 完成后暂停,等用户确认再继续 T3。
